...
#include "CalculateElasticDisplacementsModifier.h"
    ...
    _burgersContent(Vector3::Zero())
{
	...
	vectorVis()->setObjectTitle(tr("Elastic Displacements"));
...
******************************************************************************/
Future<AsynchronousModifier::ComputeEnginePtr> CalculateElasticDisplacementsModifier::createEngineWithReference(TimePoint time, ModifierApplication* modApp, PipelineFlowState input, const PipelineFlowState& referenceState, TimeInterval validityInterval)
{
...
******************************************************************************/
void CalculateElasticDisplacementsModifier::ElasticDisplacementEngine::perform()
{
	...
	if(affineMapping() != NO_MAPPING) {
		parallelForChunks(elasticDisplacements()->size(), *task(), [this](size_t startIndex, size_t count, PromiseState& promise) {
			Vector3* u = elasticDisplacements()->dataVector3() + startIndex;
			FloatType* umag = elasticDisplacementMagnitudes()->dataFloat() + startIndex;
...
******************************************************************************/
void CalculateElasticDisplacementsModifier::ElasticDisplacementEngine::emitResults(TimePoint time, ModifierApplication* modApp, PipelineFlowState& state)
{
	CalculateElasticDisplacementsModifier* modifier = static_object_cast<CalculateElasticDisplacementsModifier>(modApp->modifier());
	...
	particles->createProperty(elasticDisplacements())->setVisElement(modifier->vectorVis());
	particles->createProperty(elasticDisplacementMagnitudes());
}
