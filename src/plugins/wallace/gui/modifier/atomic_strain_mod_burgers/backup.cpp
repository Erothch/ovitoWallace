...
void AtomicStrainModBurgersEditor::createUI(const RolloutInsertionParameters& rolloutParams)
{
    ...
    for(int col = 0; col < 3; col++) {
        ...
        gridlayout->addWidget(lineEdit, row+1, col*3+0);
        gridlayout->addWidget(spinner, row+1, col*3+1);

        connect(spinner, &SpinnerWidget::spinnerValueChanged, this, &AtomicStrainModBurgersEditor::onSpinnerValueChanged);
        connect(spinner, &SpinnerWidget::spinnerDragStart, this, &AtomicStrainModBurgersEditor::onSpinnerDragStart);
        connect(spinner, &SpinnerWidget::spinnerDragStop, this, &AtomicStrainModBurgersEditor::onSpinnerDragStop);
        connect(spinner, &SpinnerWidget::spinnerDragAbort, this, &AtomicStrainModBurgersEditor::onSpinnerDragAbort);
    }
