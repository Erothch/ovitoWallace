...
void CalculateElasticStabilityModifierEditor::createUI(const RolloutInsertionParameters& rolloutParams)
{
    ...
    switch(i) {
        case(1): {
            ...
            QString labels[6] = {"C111", "C112", "C123", "C144", "C155", "C456"};
    ...
    QGroupBox* affineBox = new QGroupBox(tr("Enter transformation from typical to current directions"));
    QGridLayout* sublayout2 = new QGridLayout(affineBox);
    sublayout2->setContentsMargins(30,4,4,4);
    sublayout2->setHorizontalSpacing(0);
    sublayout2->setVerticalSpacing(2);
    sublayout2->setColumnStretch(0, 1);
    sublayout2->setColumnStretch(2, 1);
    sublayout2->setColumnStretch(4, 1);
    sublayout2->setColumnMinimumWidth(1, 4);
    sublayout2->setColumnMinimumWidth(3, 4);
    layout->addWidget(affineBox);
    AffineTransformationParameterUI* affineTransformationUI;

    for(size_t v = 0; v < 3; v++) {
        for(size_t r = 0; r < 3; r++) {
            affineTransformationUI = new AffineTransformationParameterUI(this, PROPERTY_FIELD(CalculateElasticStabilityModifier::CTransformation), r, v);
            affineTransformationUI->setEnabled(true);
            sublayout2->addLayout(affineTransformationUI->createFieldLayout(), v*2+1, r*2);
        }
    }
